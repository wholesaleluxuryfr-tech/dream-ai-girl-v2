...
// ============================================================
// V3 FEATURES - Advanced Functions
// ============================================================
...
function displayStories(stories) {
    ...
    container.innerHTML = stories.map(story => `
        <div class="story-card glass-card" onclick="viewStory('${story.id}')">
            <img src="${story.photo_url}" alt="Story">
            <div class="story-overlay">
                <div class="story-girl-name">${story.girl_name}</div>
                <div class="story-time">${story.time_ago}</div>
            </div>
        </div>
    `).join('');
}
...
function reactToStory(storyId, reaction) {
    fetch(`/api/v3/stories/${storyId}/react`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
    ...
    .then(data => {
        if (data.success) {
            showToast(`Reaction ${reaction} envoyee!`);
        }
    });
}

// ====== VOICE MESSAGES ======
function playVoiceMessage(girlId, messageType) {
    fetch(`/api/v3/voice/${girlId}/${messageType}`)
        .then(r => r.json())
        .then(data => {
            if (data.audio_url) {
                const audio = new Audio(data.audio_url);
                audio.play();
                showToast('🎤 Message vocal en cours...');
            } else {
                showToast('Simule: ' + data.message);
            }
        })
...
// ====== REACTIONS SYSTEM ======
function triggerReaction(girlId, message, affection) {
    fetch('/api/v3/reaction', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
            girl_id: girlId,
...
function displayReaction(reaction) {
    ...
    // Animate in
    setTimeout(() => reactionEl.classList.add('show'), 100);

    // Remove after 3 seconds
    setTimeout(() => {
        reactionEl.classList.remove('show');
        setTimeout(() => reactionEl.remove(), 300);
    }, 3000);
}
    ...
    container.innerHTML = challenges.map(ch => `
        <div class="challenge-card glass-card">
            ...
            <div class="challenge-progress">
                ...
                <div class="progress-text">${ch.progress}%</div>
            </div>
            <button class="challenge-btn" onclick="completeChallenge('${ch.id}')">
                Recompense: ${ch.reward.coins} 💎 + ${ch.reward.xp} XP
            </button>
        </div>
    `).join('');
}
...
function completeChallenge(challengeId) {
    fetch(`/api/v3/challenges/${challengeId}/complete`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'}
    })
    .then(r => r.json())
    .then(data => {
        if (data.success) {
            showToast(`🎉 Defi complete! +${data.reward.coins} 💎 +${data.reward.xp} XP`);
            loadChallenges();
        }
    });
}
...
// ====== MILESTONES SYSTEM ======
function checkMilestone(userId, girlId, stats) {
    fetch('/api/v3/milestones/check', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
            user_id: userId,
    ...
    modal.innerHTML = `
        <div class="milestone-content glass-card">
            ...
            <div class="milestone-reward">
                Recompense: ${milestone.reward.coins} 💎 + ${milestone.reward.xp} XP
            </div>
            <button class="milestone-close" onclick="this.parentElement.parentElement.remove()">
                Continuer
            </button>
        </div>
    `;
...
function loadGifts() {
    fetch('/api/v3/gifts')
        ...
        .catch(err => console.error('Gifts error:', err));
}
...
function displayGifts(gifts) {
    ...
    container.innerHTML = Object.entries(gifts).map(([id, gift]) => `
        <div class="gift-card glass-card" onclick="sendGift('${id}')">
            <div class="gift-icon">${gift.icon}</div>
            <div class="gift-name">${gift.name}</div>
            <div class="gift-cost">${gift.cost} 💎</div>
            <div class="gift-effect">+${gift.affection_boost} affection</div>
        </div>
    `).join('');
}

function sendGift(giftId) {
    const girlId = window.currentGirlId || 'sophie';

    fetch('/api/v3/gift/send', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
            girl_id: girlId,
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.reaction) {
                displayReaction({emoji: '😍', text: data.reaction});
            }
        } else {
            showToast(data.error || 'Erreur');
        }
...
// ====== MOOD SYSTEM ======
function loadMood(girlId) {
    fetch(`/api/v3/mood/${girlId}`)
        .then(r => r.json())
        .then(data => {
            if (data.mood) {
                displayMood(data.mood, data.data);
            }
        })
        .catch(err => console.error('Mood error:', err));
}

function displayMood(mood, moodData) {
    const moodEl = document.getElementById('moodIndicator');
    if (moodEl) {
        moodEl.innerHTML = `
            <span class="mood-icon">${moodData.icon}</span>
            <span class="mood-text">${moodData.label}</span>
        `;
        moodEl.style.backgroundColor = moodData.color;
    }
}

// ====== DIARY SYSTEM ======
function loadDiary(girlId) {
    fetch(`/api/v3/diary/${girlId}`)
        .then(r => r.json())
        .then(entry => {
            displayDiary(entry);
        })
...
function displayDiary(entry) {
    ...
    container.innerHTML = `
        <div class="diary-entry glass-card">
            ...
            <div class="diary-mood">Mood: ${entry.mood}</div>
            <div class="diary-content">${entry.content}</div>
            <div class="diary-thoughts">
                <strong>Pensees:</strong>
                ${entry.thoughts.map(t => `<p>• ${t}</p>`).join('')}
            </div>
            ${entry.special_moment ? `
                <div class="diary-special">
                    ✨ ${entry.special_moment}
                </div>
            ` : ''}
        </div>
...
// ====== V3 ENHANCED CHAT ======
function sendMessageV3(girlId, message) {
    fetch('/api/v3/chat', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
    ...
    .then(data => {
        if (data.response) {
            ...
            // Trigger reaction based on affection
            if (data.emotional_state && data.emotional_state.affection > 50) {
                triggerReaction(girlId, message, data.emotional_state.affection);
            }
        }
    })
    .catch(err => {
        console.error('Chat V3 error:', err);
        showToast('Erreur de chat');
    });
}
    ...
    if (stateEl) {
        stateEl.innerHTML = `
            <div class="emotion-bar">
                <div class="emotion-label">Affection</div>
                <div class="emotion-value">${state.affection}%</div>
            </div>
            <div class="emotion-bar">
                <div class="emotion-label">Trust</div>
                <div class="emotion-value">${state.trust}%</div>
            </div>
            <div class="emotion-bar">
                <div class="emotion-label">Excitement</div>
                <div class="emotion-value">${state.excitement}%</div>
            </div>
            <div class="emotion-bar">
                <div class="emotion-label">Intimacy</div>
                <div class="emotion-value">${state.intimacy}%</div>
            </div>
        `;
    }
}

// Expose V3 functions globally
...
window.sendMessageV3 = sendMessageV3;

console.log('✅ Dream AI Girl V3 - JavaScript features loaded');
